...
public class ModificarTipoCliente extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        boton_modificar.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                modificarTipoCliente(text_codigo.getText().toString(), text_nombre.getText().toString());
                onBackPressed();
            }
        });
    ...
    private void modificarTipoCliente (String id, String nombre) {
        BaseHelper helper = new BaseHelper(this, "Demo2", null, 1);
        SQLiteDatabase db = helper.getWritableDatabase();

        try {
            String sql = "update TIPOCLIENTE set ID='" + id +"',NOMBRE='" + nombre + "' where ID="+id;
            db.execSQL(sql);
            db .close();
